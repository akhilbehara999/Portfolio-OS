  ...
  const getIcon = (item: any) => {
    if (item.type === 'folder') {
      if (item.name === 'Desktop')
        return <LuMonitor className="text-blue-500" size={viewMode === 'grid' ? 48 : 20} />;
      if (item.name === 'Documents')
        return <LuFolder className="text-yellow-500" size={viewMode === 'grid' ? 48 : 20} />;
      if (item.name === 'Pictures')
        return <LuImage className="text-purple-500" size={viewMode === 'grid' ? 48 : 20} />;
      if (item.name === 'Projects')
        return <LuBriefcase className="text-green-500" size={viewMode === 'grid' ? 48 : 20} />;
      if (item.name === 'Downloads')
        return <LuDownload className="text-blue-400" size={viewMode === 'grid' ? 48 : 20} />;
      return <LuFolder className="text-blue-400" size={viewMode === 'grid' ? 48 : 20} />;
    }

    if (item.name.endsWith('.pdf'))
      return <LuFileText className="text-red-500" size={viewMode === 'grid' ? 40 : 20} />;
    if (item.name.endsWith('.md'))
      return <LuFileText className="text-slate-500" size={viewMode === 'grid' ? 40 : 20} />;
    if (item.name.endsWith('.png') || item.name.endsWith('.jpg'))
      return <LuImage className="text-purple-500" size={viewMode === 'grid' ? 40 : 20} />;
                  ...
                  {files.map((file) => (
                    ...
                    >
                      ...
                      <div className="text-xs text-slate-500">
                        {file.size > 0 ? `${Math.ceil(file.size / 1024)} KB` : '--'}
                      </div>
                    </motion.div>
                  ))}
                </div>
