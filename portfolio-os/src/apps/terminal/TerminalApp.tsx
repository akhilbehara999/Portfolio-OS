...
const TerminalApp: React.FC = () => {
      ...
      case 'help':
        addToOutput(
          ...
          <div className="grid grid-cols-[120px_1fr] gap-2">
            ...
            <span>Open resume app</span>
            <span className="text-yellow-400">social</span>
            <span>List social links</span>
            ...
            <span>Print current user</span>
            <span className="text-yellow-400">date</span>
            <span>Print current date</span>
            <span className="text-yellow-400">echo [txt]</span>
            ...
            <span className="text-yellow-400">open [app]</span>
      ...
      case 'contact':
        addToOutput(
          ...
          <div>
            <div>
              Email:{' '}
              <a href={`mailto:${PORTFOLIO_DATA.contact.email}`} className="text-blue-400">
          ...
          addToOutput(
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2">
              {children.map((child: any) => (
                <span
                  key={child.id}
                  className={child.type === 'folder' ? 'text-blue-400 font-bold' : 'text-white'}
                >
                  {child.name}
                  {child.type === 'folder' ? '/' : ''}
                </span>
 ...
 |_|  |_|\\___/ `}
            ...
            <div className="flex flex-col justify-center">
              ...
              <div>-----------------------</div>
              <div>
                <span className="text-yellow-400">OS</span>: PortfolioOS v1.0.0
              </div>
              <div>
              ...
              <div>
                <span className="text-yellow-400">Kernel</span>: React 19.0.0
              </div>
              <div>
                <span className="text-yellow-400">Uptime</span>: {Math.floor(uptime / 60)} mins
              </div>
              <div>
                <span className="text-yellow-400">Packages</span>: {APP_REGISTRY.length} (dpkg)
              </div>
              <div>
                <span className="text-yellow-400">Shell</span>: TerminalJS
              </div>
              <div>
                <span className="text-yellow-400">Theme</span>:{' '}
                {useThemeStore.getState().currentTheme.name}
              </div>
              <div>
                <span className="text-yellow-400">Device</span>: {deviceMode}
              </div>
      ...
      case 'theme': {
        if (args[0]) {
          const theme = THEME_PRESETS.find(
            (t) => t.id === args[0] || t.name.toLowerCase() === args[0].toLowerCase()
          );
          if (theme) {
            setTheme(theme.id);
            ...
            addToOutput(
              'error',
              `Theme '${args[0]}' not found. Available: ${THEME_PRESETS.map((t) => t.id).join(', ')}`
            );
          }
        } else {
          addToOutput(
            'response',
            `Current theme: ${useThemeStore.getState().currentTheme.name}\nAvailable: ${THEME_PRESETS.map((t) => t.id).join(', ')}`
          );
        }
        break;
      }
      ...
      case 'wallpaper': {
        if (args[0]) {
          ...
          addToOutput(
            'response',
            `Available wallpapers: ${WALLPAPERS.map((w) => w.id).join(', ')}`
          );
        }
        break;
      }
      ...
      case 'open': {
        if (args[0]) {
          const app = APP_REGISTRY.find(
            (a) => a.id === args[0] || a.name.toLowerCase() === args[0].toLowerCase()
          );
          if (app) {
            launchApp(app.id);
      ...
      >
        {output.map((line) => (
          <div key={line.id} className="mb-1 break-words">
            {line.type === 'command' && (
              <div className="flex">
                <span className="text-blue-400 mr-2 min-w-fit">
                  visitor@portfolio-os:{line.path}$
                </span>
            ...
            {line.type === 'ascii' && (
              <pre className="text-green-500 font-bold leading-none">{line.content}</pre>
            )}
          </div>
        ))}
