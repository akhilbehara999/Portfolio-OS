...
export const Taskbar: React.FC<TaskbarProps> = ({ onToggleNotifications }) => {
  ...
  const taskbarItems = useMemo(() => {
    ...
    const items = pinnedAppIds.map(id => {
      ...
      >
        ...
        <div className="flex items-center gap-1 h-full px-4">
           {taskbarItems.map((item) => {
             ...
             return (
               <div key={item.id} className="relative group flex flex-col items-center justify-center h-full">
                 {/* Tooltip */}
                 <AnimatePresence>
                     ...
                     >
                       ...
                       {item.running && (
                         <div className="flex gap-1 mt-1 justify-center">
                            ...
                            <div className={`w-8 h-5 rounded-sm border ${isDarkMode ? 'border-gray-600 bg-gray-700' : 'border-gray-300 bg-gray-100'}`} />
                         </div>
                       )}
                     </motion.div>
                   )}
